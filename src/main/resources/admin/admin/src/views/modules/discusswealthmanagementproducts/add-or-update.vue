	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="No.Comment content" prop="content">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="No.Comment content" prop="content">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.content}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="Reply to content" prop="reply">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.reply" label="Reply to content" prop="reply">
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusswealthmanagementproducts/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusswealthmanagementproducts/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusswealthmanagementproductsCrossAddOrUpdateFlag = false;
						 this.parent.search();
