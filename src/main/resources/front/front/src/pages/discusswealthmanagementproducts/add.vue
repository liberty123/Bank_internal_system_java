...
<div :style='{"width":"90%","padding":"30px 0px 40px","margin":"10px auto","position":"relative","background":"rgba(255,255,255,.0)"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="head sculpture" v-if="type!='cross' || (type=='cross' && !ro.avatarurl)" prop="avatarurl">
            <file-upload
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="head sculpture" prop="avatarurl">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="Username" prop="nickname">
            <el-input v-model="ruleForm.nickname" 
                placeholder="Username" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="No.Comment content" prop="content">
            <el-input
              type="textarea"
              :rows="8"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="Reply to content" prop="reply">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="Reply to content"
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          refid: [
            { required: true, message: '关联表idCannot be empty', trigger: 'blur' },
          ],
          userid: [
          ...
          content: [
            { required: true, message: 'No.Comment contentCannot be empty', trigger: 'blur' },
          ],
          reply: [
          ],
        },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('discusswealthmanagementproducts/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('discusswealthmanagementproducts/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('discusswealthmanagementproducts/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
