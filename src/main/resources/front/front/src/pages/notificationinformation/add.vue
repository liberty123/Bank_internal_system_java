...
<div :style='{"width":"90%","padding":"30px 0px 40px","margin":"10px auto","position":"relative","background":"rgba(255,255,255,.0)"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="notifier" prop="employeeid">
            <el-input v-model="ruleForm.employeeid" 
                placeholder="notifier" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="client number" prop="customernumber">
            <el-select  @change="customernumberChange" v-model="ruleForm.customernumber" placeholder="Please select client number">
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="Notification Time" prop="notificationtime" >
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="notification content" prop="notificationcontent">
            <el-input
              type="textarea"
              :rows="8"
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/customer/customernumber', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.customernumberOptions = res.data.data;
          }
        });
      },
      // 下二随
      customernumberChange () {
        this.$http.get('follow/customer/customernumber?columnValue=' + this.ruleForm.customernumber, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('notificationinformation/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('notificationinformation/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('notificationinformation/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
