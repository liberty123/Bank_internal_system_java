...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Exam Type</div>
        <el-select v-model="formSearch.examtype" placeholder="Please select Exam Type" :clearable="true">
          <el-option v-for="(item, index) in examtypeOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('examnotice','add')" :style='{"cursor":"pointer","border":"8px inset #ffc766","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"32px","height":"45px"}' type="primary" @click="add('/index/examnoticeAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>add</el-button>
    </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.examtype != '') searchWhere.examtype = this.formSearch.examtype;
        this.$http.get('examnotice/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/examnoticeDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
