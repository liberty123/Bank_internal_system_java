...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>product  number</div>
        <el-input v-model="formSearch.productnumber" placeholder="product  number" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Issuing institution</div>
        <el-input v-model="formSearch.issuinginstitution" placeholder="Issuing institution" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('wealthmanagementproducts','add')" :style='{"cursor":"pointer","border":"8px inset #ffc766","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"32px","height":"45px"}' type="primary" @click="add('/index/wealthmanagementproductsAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>add</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"20px 0 10px","background":"none","flex":"1"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"1px solid #e6e6e6","padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"rgba(255,255,255,.8)","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","boxShadow":"0px 56px 36px -60px #999","objectFit":"cover","display":"block","height":"275px"}' v-if="item.cover && item.cover.substr(0,4)=='http'" :src="item.cover" class="image" />
				<img :style='{"width":"100%","boxShadow":"0px 56px 36px -60px #999","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.cover?item.cover.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"4px 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#3d74c0","width":"100%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.productclassification}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.productnumber != '') searchWhere.productnumber = '%' + this.formSearch.productnumber + '%';
        if (this.formSearch.issuinginstitution != '') searchWhere.issuinginstitution = '%' + this.formSearch.issuinginstitution + '%';
        if (this.curFenlei != 'All') searchWhere.productclassification = this.curFenlei;
        this.$http.get('wealthmanagementproducts/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/wealthmanagementproductsDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
