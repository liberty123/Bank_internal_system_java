    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="Loan method" prop="loanmethod">
            <el-input v-model="ruleForm.loanmethod" 
                placeholder="Loan method" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('loanmethods/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('loanmethods/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('loanmethods/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
