		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 auto 20px","borderRadius":"3px","textAlign":"center","background":"none","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"50px","fontSize":"16px","fontWeight":"500","height":"50px"}' class="item-label">product  number</label>
						<el-input v-model="searchForm.productnumber" placeholder="product  number" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"50px","fontSize":"16px","fontWeight":"500","height":"50px"}' class="item-label">Issuing institution</label>
						<el-input v-model="searchForm.issuinginstitution" placeholder="Issuing institution" clearable></el-input>
					</div>
				...
				<el-row :style='{"padding":"16px 12px 10px","margin":"0 0 0px","borderColor":"#e2f0e2","flexWrap":"wrap","background":"#fff","borderWidth":"1px 1px 0 1px","display":"flex","width":"auto","borderStyle":"solid","justifyContent":"flex-end","zIndex":"999"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#666","color":"#333","outline":"none","borderRadius":"0px","background":"#f9c535","borderWidth":"0 0 0px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('wealthmanagementproducts','add')" type="success" @click="addOrUpdateHandler()">add</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#666","color":"#333","outline":"none","borderRadius":"0px","background":"#f9c535","borderWidth":"0 0 0px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('wealthmanagementproducts','del')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">del</el-button>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('wealthmanagementproducts','view')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="cover" width="200" label="cover">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="fundraisingmethod"
					label="Fundraising method">
						<template slot-scope="scope">
							{{scope.row.fundraisingmethod}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="fundraisingcurrency"
					label="Fundraising currency">
						<template slot-scope="scope">
							{{scope.row.fundraisingcurrency}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="risklevel"
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="investmentnature"
					label="Investment nature">
						<template slot-scope="scope">
							{{scope.row.investmentnature}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="actualdays"
					...
					<el-table-column width="300" label="operate">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('wealthmanagementproducts','view')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">details</el-button>
							<el-button :style='{"border":"0px solid #06d5dd","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat left center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('wealthmanagementproducts','modify')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">modify</el-button>


							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if="isAuth('wealthmanagementproducts','View comments')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">View comments</el-button>



							<el-button :style='{"border":"0px solid #c00","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#c00","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/c5ea125f385740a3a3b9aed07f5dfe2c.png) no-repeat left 6px / 20px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('wealthmanagementproducts','del') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">del</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.productnumber!='' && this.searchForm.productnumber!=undefined){
            params['productnumber'] = '%' + this.searchForm.productnumber + '%'
          }
           if(this.searchForm.issuinginstitution!='' && this.searchForm.issuinginstitution!=undefined){
            params['issuinginstitution'] = '%' + this.searchForm.issuinginstitution + '%'
          }
      this.$http({
	...
	this.$router.push({path:'/discusswealthmanagementproducts',query:{refid:id}});
    },
    // download
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`Are you sure to proceed with[${id ? "del" : "batchdel"}]operate?`, "tiitle", {
        ...
        this.$http({
          url: "wealthmanagementproducts/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
