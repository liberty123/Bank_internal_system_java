		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 auto 20px","borderRadius":"3px","textAlign":"center","background":"none","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"50px","fontSize":"16px","fontWeight":"500","height":"50px"}' class="item-label">notifier</label>
						<el-input v-model="searchForm.employeeid" placeholder="notifier" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"50px","fontSize":"16px","fontWeight":"500","height":"50px"}' class="item-label">Notification Time</label>
						<el-date-picker v-model="searchForm.notificationtimestart" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss" placeholder="Notification Time start"></el-date-picker>
						to
						<el-date-picker v-model="searchForm.notificationtimeend" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss" placeholder="Notification Time end"></el-date-picker>
					</div>
				...
				<el-row :style='{"padding":"16px 12px 10px","margin":"0 0 0px","borderColor":"#e2f0e2","flexWrap":"wrap","background":"#fff","borderWidth":"1px 1px 0 1px","display":"flex","width":"auto","borderStyle":"solid","justifyContent":"flex-end","zIndex":"999"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#666","color":"#333","outline":"none","borderRadius":"0px","background":"#f9c535","borderWidth":"0 0 0px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('notificationinformation','add')" type="success" @click="addOrUpdateHandler()">add</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#666","color":"#333","outline":"none","borderRadius":"0px","background":"#f9c535","borderWidth":"0 0 0px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('notificationinformation','del')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">del</el-button>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('notificationinformation','view')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="operate">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('notificationinformation','view')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">details</el-button>
							<el-button :style='{"border":"0px solid #06d5dd","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat left center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('notificationinformation','modify')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">modify</el-button>





							<el-button :style='{"border":"0px solid #c00","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#c00","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/c5ea125f385740a3a3b9aed07f5dfe2c.png) no-repeat left 6px / 20px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('notificationinformation','del') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">del</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
    ...
    params['sort'] = 'notificationtime';
    params['order'] = 'desc';
           if(this.searchForm.notificationtimestart!='' && this.searchForm.notificationtimestart!=undefined ){
            params['notificationtimestart'] = this.searchForm.notificationtimestart
          }
          if(this.searchForm.notificationtimeend!='' && this.searchForm.notificationtimeend!=undefined){
            params['notificationtimeend'] = this.searchForm.notificationtimeend
          }
      this.$http({
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`Are you sure to proceed with[${id ? "del" : "batchdel"}]operate?`, "tiitle", {
        ...
        this.$http({
          url: "notificationinformation/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
