			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="operate">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #06d5dd","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat left center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('discusswealthmanagementproducts','modify')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">modify</el-button>


							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if="isAuth('discusswealthmanagementproducts','View comments')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">View comments</el-button>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: "discusswealthmanagementproducts/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.$router.push({path:'/discussdiscusswealthmanagementproducts',query:{refid:id}});
    },
    // download
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`Are you sure to proceed with[${id ? "del" : "batchdel"}]operate?`, "tiitle", {
        ...
        this.$http({
          url: "discusswealthmanagementproducts/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
