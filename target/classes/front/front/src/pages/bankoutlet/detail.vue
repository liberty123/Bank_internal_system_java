...
<div>
	...
	<div class="detail-preview" :style='{"width":"90%","margin":"10px auto","position":"relative","background":"none"}'>
		<div class="attr" :style='{"border":"1px solid #e6e6e6","padding":"10px 0 0px 0px","borderRadius":"4px","flexWrap":"wrap","background":"rgba(255,255,255,.6)","display":"flex","position":"relative","justifyContent":"space-between"}'>
			...
			<div class="info" :style='{"minHeight":"500px","padding":"20px 20px 20px","margin":"0px 0px 0 0px","background":"rgba(255,255,255,.0)","flex":"1","width":"48%","order":"2"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"none","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#2693b3","textAlign":"right","width":"122px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>Bank type</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.banktype}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"none","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"none","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#2693b3","textAlign":"right","width":"122px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>Telephone</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.contactphone}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"none","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#2693b3","textAlign":"right","width":"122px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>business hours</div>
					<div  :style='{"width":"498px","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.businesshours}}</div>
				</div>
		...
		<el-tabs class="detail" :style='{"border":"1px solid #e6e6e6","width":"100%","boxShadow":"0 1px 6px 0 rgba(0, 0, 0, .1)","padding":"10px","margin":"20px 0 0","background":"rgba(255,255,255,1)"}' v-model="activeName" type="border-card">
			<el-tab-pane label="brief introduction" name="first">
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.detail.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.bankname;
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'bankoutlet', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discussbankoutlet/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussbankoutlet/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
