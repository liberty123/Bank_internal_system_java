...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>notifier</div>
        <el-input v-model="formSearch.employeeid" placeholder="notifier" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>Notification Time</div>
        <el-date-picker 
          v-model="timeRange"
          type="datetimerange"
          range-separator="to"
		  :style='{"border":"8px solid #5fa6d0","padding":"3px 10px","outline":"none","borderRadius":"8px","background":"#fff","width":"auto","justifyContent":"center","height":"45px"}'
          start-placeholder="Notification Time start"
          end-placeholder="Notification Time end"
	  ...
	  <el-button v-if="isAuth('notificationinformation','add')" :style='{"cursor":"pointer","border":"8px inset #ffc766","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"32px","height":"45px"}' type="primary" @click="add('/index/notificationinformationAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>add</el-button>
    </el-form>
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.timeRange.length > 0) {
          searchWhere.notificationtimestart = this.timeRange[0];
          searchWhere.notificationtimeend = this.timeRange[1];
        }
        params['sort'] = 'notificationtime';
        params['order'] = 'desc';
        this.$http.get('notificationinformation/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/notificationinformationDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
